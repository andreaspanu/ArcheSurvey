...
import it.archesurvey.app.data.repository.RoomSurveyRepository
...
class AppContainer(context: Context) {
    private val database = Room.databaseBuilder(
        ...
        ArcheSurveyDatabase.MIGRATION_1_2,
        ArcheSurveyDatabase.MIGRATION_2_3
    ).build()

    private val projectRepository: ProjectRepository = RoomProjectRepository(database.projectDao())
    private val surveyRepository: SurveyRepository = RoomSurveyRepository(database.surveyDao())
